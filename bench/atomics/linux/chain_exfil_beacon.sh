...
# `suspicious_process_read_sensitive_and_connected` (OCSF 9002). The lolbin-in-/tmp
...
timeout 5 /tmp/nc -c "cat /etc/shadow >/dev/null 2>&1; exec 3<>/dev/tcp/${host}/${port}" 2>/dev/null \
  || timeout 5 bash -c "cat /etc/shadow >/dev/null 2>&1; exec 3<>/dev/tcp/${host}/${port}" 2>/dev/null \
  || true
